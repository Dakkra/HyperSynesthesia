...
class DSPTest {
	...
	void processLight_computesPeakAndRms() {
		...
		// This is the square root of the sum of the squared values over n where
		// n is the number of values. In other words, n is the array/sample
		// buffer length.
		assertEquals( 4, dsp.getPeak(), "Peak loudness should be absolute max of samples" );
		// Intentionally not simplified
		double expectedRms = Math.sqrt( (0 + 9 + 16 + 4) / 4.0 );
		assertEquals( expectedRms, dsp.getRMS(), TOLERANCE );
	...
	void processFull_producesNormalizedSpectrum() {
		...
		assertEquals( 1.0, max, TOLERANCE, "Spectrum should be normalized such that max is 1" );
